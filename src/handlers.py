...
import re
from typing import Any, Callable, Dict, List, NamedTuple, Optional, Sized, Tuple, Union
...
class IndexTupleCommandHandler(MethodCommandHandler):
    ...
    def handle(self, params: str) -> Payload:
        ...
        if index < 0 and all_params:
            raise Exception(
                f"Cannot list all elements and parameters at the same '{params}' request"
            )

        result = self.get_value()
        assert isinstance(result, list)
        if index < 0:
            return list_from_array_of_namedtupes(
                result, param, params, index_str.endswith(";")
            )
        else:  # index selected
            try:
                elt = result[index]
                if all_params:
                    return string_from_dict_optionally(
                        elt._asdict(), param.endswith(";")
                    )
                elif param in elt._fields:
                    return getattr(elt, param)
                else:
                    raise Exception(
                        "Parameter '" + param + "' in '" + params + "' is not supported"
                    )
            except IndexError:
...
class IndexOrTotalCommandHandler(CommandHandler):
    ...
    def handle(self, params: str) -> Payload:
        ...
        if params == "*":
            ...
            raise Exception(
                "Parameter '" + params + "' in '" + self.name + "' is not supported"
            )
...
class IndexOrTotalTupleCommandHandler(MethodCommandHandler):
    ...
    def handle(self, params: str) -> Payload:
        ...
        if index_str == "*":
            ...
            raise Exception(
                "Element '" + index_str + "' in '" + params + "' is not supported"
            )

        if not total and index < 0 and all_params:
            raise Exception(
                "Cannot list all elements and parameters at the same '"
                + params
                + "' request"
            )
...
class DiskUsageCommandHandler(MethodCommandHandler):
    ...
    def handle(self, params: str) -> Payload:
        ...
        raise Exception(
            "Parameter '" + param + "' in '" + self.name + "' is not supported"
        )
        ...
        if source == "*" or source == "*;":
            ...
            if label == "" and param == "":
                ...
                temps = (
                    llist[int(label)]
                    if label.isdigit()
                    else next((x for x in llist if x.label == label), None)
                )
                if temps is None:
                    raise Exception(
                        "Device '" + label + "' in '" + self.name + "' is not supported"
                    )
                if param == "":
                    return temps.current
                elif param == "*" or param == "*;":
                    return string_from_dict_optionally(
                        temps._asdict(), param.endswith(";")
                    )
                else:
                    return temps._asdict()[param]

        raise Exception(
            "Sensor '" + source + "' in '" + self.name + "' is not supported"
        )
...
class ProcessesCommandHandler(CommandHandler):
    ...
    def handle(self, params: str) -> Payload:
        ...
        if process in ("*", "*;"):
            ...
            result = {
                p.pid: self.get_process_value(p, param, params)
        ...
        return self.get_process_value(psutil.Process(pid), param, params)
    ...
    @staticmethod
    def get_process_value(process: psutil.Process, params: str, all_params: str) -> Any:
...
handlers = {
    ...
    "cpu_percent": type(
        ...
        {"get_value": lambda self, total: psutil.cpu_percent(percpu=not total)},
    )("cpu_percent"),
    "cpu_times_percent": type(
        "CpuTimesPercentCommandHandler",
        (IndexOrTotalTupleCommandHandler, object),
        {"get_value": lambda self, total: psutil.cpu_times_percent(percpu=not total)},
    )("cpu_times_percent"),
    ...
    "disk_io_counters": type(
        ...
        (IndexOrTotalTupleCommandHandler, object),
        {"get_value": lambda self, total: psutil.disk_io_counters(perdisk=not total)},
    )("disk_io_counters"),
    "net_io_counters": type(
        "NetIOCountersCommandHandler",
        (NameOrTotalTupleCommandHandler, object),
        {"get_value": lambda self, total: psutil.net_io_counters(pernic=not total)},
    )("net_io_counters"),
...
class ProcessPropertiesCommandHandler(ProcessCommandHandler):
    ...
    def get_value(self, process: psutil.Process) -> Payload:
        ...
        for k, handler in process_handlers.items():
            if (
                hasattr(handler, "method") and handler.method is not None
            ):  # property is defined for current OS
                try:
                    if isinstance(handler, ProcessMethodCommandHandler):
                        ...
                        if isinstance(
                            handler, ProcessMethodIndexCommandHandler
...
process_handlers = {
    ...
    "pid": type(
        "ProcessPidCommandHandler",
        (ProcessMethodCommandHandler, object),
        {"get_value": lambda self, process: process.pid},
    )("pid"),
...
def list_from_array_of_namedtupes(
    array_of_namedtupes: Union[List[Any], NamedTuple], key, func, join: bool = False
) -> Union[List[Any], str]:
    result = list()
...
def dict_from_dict_of_namedtupes(
    dict_of_namedtupes: Dict[str, NamedTuple], key: str, func, join=False
...
def string_from_dict_optionally(
    d: Dict[Any, Any], join: bool
